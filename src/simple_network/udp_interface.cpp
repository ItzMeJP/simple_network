...
namespace udp_interface {
    ...
    void UDPBasis::setTarget(int _protocol, std::string ip_address, unsigned short _port){
        setProtocol(_protocol);
        setAddress(ip_address);
        setPort(_port);
    }

    std::string UDPBasis::getOutputMSG(){
        return output_string_;
    }

    void UDPBasis::setProtocol(int _in) {
        protocol_ = _in;
    }

    void UDPBasis::setAddress(std::string _in) {
        ip_address_ = _in;
    }

    void UDPBasis::setPort(unsigned short _in) {
        port_ = _in;
    }
    ...
    UDPServer::UDPServer(int _protocol, std::string _ip_address, unsigned short _port){
        ...
        socket_ = std::make_shared<boost::asio::ip::udp::socket>(io_service_,
                                                                 boost::asio::ip::udp::endpoint{boost::asio::ip::make_address(ip_address_),
                                                                                                port_});
        startReceive();
    }
    ...
    void UDPServer::generateTimestamp(){
        ...
        std::string ss(std::ctime(&now),std::ctime(&now)+sizeof(char)*(strlen(std::ctime(&now))-1)); //just to remove a \n
    ...
    void UDPServer::receiveHandler(const boost::system::error_code &error, std::size_t ){
        ...
        if (!error || error == boost::asio::error::message_size) {
            ...
            unsigned int packetSize = socket_->receive_from(boost::asio::buffer(buffer, available), current_client_info_, 0, ec);
            generateTimestamp();
    ...
    void UDPServer::spinOnce(){
        try {
            io_service_.run_one(); // blocking once but guarantee a new msg
        }
        catch (std::exception &e) {
            output_string_ = e.what();
        }
    ...
    void UDPServer::spinPoll(){
        try {
            io_service_.poll(); // blocking once but guarantee a new msg
        }
        catch (std::exception &e) {
            output_string_ = e.what();
        }
    ...
    void UDPServer::spinForever(){
        try {
            io_service_.run(); // blocking once but guarantee a new msg
        }
        catch (std::exception &e) {
            output_string_ = e.what();
        }
